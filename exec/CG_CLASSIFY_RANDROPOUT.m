...
function [f, df] = CG_CLASSIFY_RANDROPOUT(VV,Dim,XX,target,n_layers);
...
w{1} = reshape(VV(1:(Dim(1)+1)*Dim(2)),Dim(1)+1,Dim(2));
        ...
        % For dropout %
        rand_vec = round(rand(1,Dim(2)));
        dropout_mat = repmat(rand_vec,Dim(1)+1,1);
        % End dropout %
        
		wprobs{1} = 1./(1 + exp(-XX*(w{1}.*dropout_mat))); 
		wprobs{1} = [wprobs{1}  ones(N,1)];

for k = 2:n_layers
	    % For dropout %
        rand_vec = round(rand(1,Dim(k+1)));
        dropout_mat = repmat(rand_vec,Dim(k)+1,1);
        % End dropout %
		w{k} = reshape(VV(xxx+1:xxx+(Dim(k)+1)*Dim(k+1)),Dim(k)+1,Dim(k+1));
		xxx=xxx+(Dim(k)+1)*Dim(k+1);
		wprobs{k} = 1./(1 + exp(-wprobs{k-1}*(w{k}.*dropout_mat))); 
